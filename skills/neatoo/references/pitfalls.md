...
| Creating `EntityLazyLoad<T>` in `[Fetch]` instead of constructor | Loader delegate lost during serialization — client gets `EntityLazyLoad` with no loader, `await` throws `InvalidOperationException` | Create `EntityLazyLoad<T>` in the **constructor** with a lambda that captures DI factory and references `this.Id` (resolved at load-time). See [lazy-loading.md](lazy-loading.md) |
| Using `OnDeserialized`/`ReinitializeLazyLoaders` to recreate `EntityLazyLoad` | Unnecessary complexity — the converter now merges deserialized state into constructor-created instances, preserving the loader | Remove `OnDeserialized`/`InitializeLazyLoaders` workarounds; move `EntityLazyLoad` creation to the constructor |
