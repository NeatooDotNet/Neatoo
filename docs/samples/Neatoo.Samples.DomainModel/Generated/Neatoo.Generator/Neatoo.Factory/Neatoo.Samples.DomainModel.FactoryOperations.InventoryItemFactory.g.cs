...
namespace Neatoo.Samples.DomainModel.FactoryOperations
{
    ...
    internal class InventoryItemFactory : FactorySaveBase<IInventoryItem>, IFactorySave<InventoryItem>, IInventoryItemFactory
    {
        ...
        public virtual Task<IInventoryItem?> LocalSave(IInventoryItem target, CancellationToken cancellationToken = default)
        {
            if (target.IsDeleted)
            {
                if (target.IsNew)
                {
                    return Task.FromResult(default(IInventoryItem));
                }
