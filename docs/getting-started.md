...
- `IsSavable` - Computed property: true if `(IsModified || IsNew) && IsValid && !IsBusy` — a created entity is savable without being modified. Only accessible through the `IEntityRoot` interface (aggregate roots), not `IEntityBase` (child entities)
