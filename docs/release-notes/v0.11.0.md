...
**Key Features:**
- ~~`.Value` property never triggers a load - always returns current state~~ **Changed in a later release:** `.Value` now auto-triggers a fire-and-forget load when unloaded and a loader is present. Returns `null` synchronously; `PropertyChanged` fires when load completes. `IsLoading`/`IsLoaded` remain side-effect-free.
- Explicit loading via `await entity.History` or `.LoadAsync()` (still works)
