...
**Key Features:**
- `.Value` property never triggers a load -- always returns current state (`null` if not loaded). *(This was temporarily changed to auto-trigger in [v0.21.0](v0.21.0.md), then reverted back to passive read in [v0.23.0](v0.23.0.md) based on real-world feedback.)*
- Explicit loading via `.LoadAsync()`. *(The `await entity.History` syntax via `GetAwaiter()` was removed in [v0.23.0](v0.23.0.md) -- use `await entity.History.LoadAsync()` instead.)*
