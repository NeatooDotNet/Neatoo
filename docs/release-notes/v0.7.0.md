...
## Subsequent Fix: Hash-Based IDs (0.26.x)

The ordinal-based scheme introduced in 0.7.0 had a collision bug: derived classes started ordinals at `1u` independently of their base class, causing `ArgumentException` when a derived class registered rules alongside inherited base class rules. This was fixed by replacing sequential ordinals with FNV-1a hash-based IDs. Generated `GetRuleId` now outputs hex hash literals (`0xHHHHHHHHu`) instead of sequential integers. See [Rule ID Collision in Inheritance](../todos/completed/rule-id-collision-in-inheritance.md).
