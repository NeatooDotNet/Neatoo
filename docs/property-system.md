...
Each partial property gets a strongly-typed backing field property generated by the source generator. This provides direct access to the property wrapper with full type safety:

<!-- pseudo:property-backing-fields -->
```csharp
// User writes:
public partial string? Name { get; set; }
...
Use backing field properties to:
- Configure lazy loading
- Access property metadata directly
- Check property-specific state without string-based indexer

<!-- pseudo:backing-field-usage -->
```csharp
public Person(IEntityBaseServices<Person> services, IPhoneDbContext context)
    : base(services)
{
    // Configure lazy loading on the backing field
    PhonesProperty.OnLoad = async () => await context.LoadPhones(this.Id);

    // Access property state directly
    bool isValid = NameProperty.IsValid;
    bool isBusy = EmailProperty.IsBusy;
}
```
<!-- /snippet -->
...
public interface IValidateProperty : INotifyPropertyChanged, INotifyNeatooPropertyChanged
{
    ...
    bool IsLoaded { get; }      // True if value has been loaded (or no OnLoad configured)
    Task? LoadTask { get; }     // Task for in-progress lazy load
    Task LoadAsync();           // Explicitly trigger lazy load
}
```
<!-- /snippet -->

### IValidateProperty<T> Interface

The generic interface adds strongly-typed value access and lazy loading configuration:

<!-- pseudo:ivalidateproperty-generic-interface -->
```csharp
public interface IValidateProperty<T> : IValidateProperty
{
    new T? Value { get; set; }

    // Lazy loading handler - set in constructor
    Func<Task<T?>>? OnLoad { get; set; }

    // Explicit load returning the loaded value
...
Properties support lazy loading for child objects that should be loaded on-demand rather than upfront:

<!-- pseudo:lazy-loading-basic -->
```csharp
public Person(IEntityBaseServices<Person> services, IPhoneDbContext context, IPhoneListFactory factory)
    : base(services)
{
    // Configure lazy loading on the backing field property
    PhonesProperty.OnLoad = async () =>
    {
        var phoneDtos = await context.LoadPhones(this.Id);
        return factory.Fetch(phoneDtos);
    };
}
```
<!-- /snippet -->
...
When you access a lazy property, loading happens in the background:

<!-- pseudo:lazy-loading-access -->
```csharp
// First access triggers the load (fire-and-forget)
var phones = person.Phones;  // Returns null immediately, load starts in background

// Wait for the load to complete
await person.WaitForTasks();

// Now the value is available
phones = person.Phones;  // Returns loaded PhoneList
```
<!-- /snippet -->

### Explicit Loading

To wait for a specific property to load:

<!-- pseudo:explicit-loading -->
```csharp
// Load and await the result
var phones = await person.PhonesProperty.LoadAsync();

// Or check load state
if (!person.PhonesProperty.IsLoaded)
{
    await person.PhonesProperty.LoadAsync();
}
```
<!-- /snippet -->
...
If lazy loading fails, the property becomes invalid with a broken rule:

<!-- pseudo:load-failure-handling -->
```csharp
// Attempt to load
_ = person.Phones;  // Triggers load
await person.WaitForTasks();

// Check if load succeeded
if (!person.PhonesProperty.IsValid)
{
    // Load failed - property has error message
    var message = person.PhonesProperty.PropertyMessages.First().Message;
    // e.g., "Failed to load: Database connection failed"
}
```
<!-- /snippet -->
...
|----------|-------------|
| **Fire-and-forget** | Getter returns immediately; load runs in background |
| **Single load** | Once loaded, accessing the property doesn't trigger another load |
| **IsBusy during load** | `property.IsBusy` returns `true` while loading |
| **Error as broken rule** | Load failures surface as validation errors on the property |
| **Not serialized** | `OnLoad` is not serialized; reconfigure after deserialization |
| **Thread-safe** | Concurrent access triggers only one load operation |
