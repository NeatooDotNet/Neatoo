...
surface per design.md's Doc/Skill Touchpoints: the neatoo skill (Key Properties table, the
Why, the COMMON MISTAKE about `MarkModified()` not being needed for savability — copied to
`~/.claude/skills/neatoo/`), `docs/guides/change-tracking.md` (full treatment) and the other
