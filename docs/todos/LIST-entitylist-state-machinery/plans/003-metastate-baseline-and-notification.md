...
| `FactoryComplete(Fetch)` / `(Create)` announce nothing | `EntityListBaseTests.FactoryComplete_Fetch_AnnouncesNothing` and `...FactoryComplete_Create_AnnouncesNothing` | unit | Pinned (both). `Create` was initially covered only "by construction"; the test gate correctly noted that structural unreachability is a property of the current guard, not of the design, so a real test was added |
...
| Revert fails the new tests; suite green, no intent weakened | **Two revert runs at different tree states**, not one: `reviews/003-revert-unit.log` (`Total: 1838`) ran before the `LocalSaveLifecycle` fixture existed and shows 1 failure, `SaveCarryingDeletions_ThenChildEdit_AnnouncesIsModified`; `reviews/003-revert-integration.log` ran after, filtered to that class (`Total: 2`) and shows 1 failure, `LocalSaveCarryingDeletions_ThenChildEdit_MakesAggregateSavableAgain`. Both logs carry the failing test names and assertion messages. Solution run after the gate recommendations: 1846 passed / 0 failed / 2 skipped, plus Samples 254, BaseGenerator 42, Person.DomainModel 55; `Design.Tests` 130 / 0 | unit | Pinned |
