...
namespace Samples
{
    public partial class ValidationAsyncOrder
    {
        ...
        protected override uint GetRuleId(string sourceExpression)
        {
            ...
            {
                @"async (order, token) => { if (string.IsNullOrEmpty(order.ProductCode)) return """"; var inStock = await inventoryService.IsInStockAsync(order.ProductCode, token); return inStock ? """" : ""Product is out of stock""; }" => 1u,
        ...
        /// </summary>
        protected override void InitializePropertyBackingFields(IPropertyFactory<Samples.ValidationAsyncOrder> factory)
        {
