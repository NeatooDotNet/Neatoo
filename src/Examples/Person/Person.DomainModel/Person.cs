using Neatoo;
using Neatoo.RemoteFactory;
using Person.Dal;
using System.ComponentModel;
using System.ComponentModel.DataAnnotations;

namespace DomainModel;

public partial interface IPerson : IEntityRoot
{
    // Not Empty - Properties auto-generated by BaseGenerator (Roslyn)
    EntityLazyLoad<IPersonPhoneList> PersonPhoneList { get; }
}

[Factory]
[AuthorizeFactory<IPersonAuth>]
internal partial class Person : EntityBase<Person>, IPerson
{
    private readonly IEntityLazyLoadFactory _lazyLoadFactory;
    private readonly IPersonPhoneListFactory _personPhoneListFactory;

    public Person(IEntityBaseServices<Person> editBaseServices,
                  IUniqueNameRule uniqueNameRule,
                  IPersonPhoneListFactory personPhoneListFactory,
                  IEntityLazyLoadFactory lazyLoadFactory) : base(editBaseServices)
    {
        _lazyLoadFactory = lazyLoadFactory;
        _personPhoneListFactory = personPhoneListFactory;
        RuleManager.AddRule(uniqueNameRule);

        PersonPhoneList = lazyLoadFactory.Create<IPersonPhoneList>(async () =>
        {
            return await _personPhoneListFactory.Fetch(this.Id ?? Guid.Empty);
        });
    }

    public partial Guid? Id { get; set; }

    [DisplayName("First Name*")]
    [Required(ErrorMessage = "First Name is required")]
    public partial string? FirstName { get; set; }

    [DisplayName("Last Name*")]
    [Required(ErrorMessage = "Last Name is required")]
    public partial string? LastName { get; set; }

    [DisplayName("Email Address")]
    public partial string? Email { get; set; }

    [DisplayName("Notes")]
    public partial string? Notes { get; set; }

    public partial EntityLazyLoad<IPersonPhoneList> PersonPhoneList { get; set; }

    public void MapFrom(PersonEntity personEntity)
    {
        this.Id = personEntity.Id;
        this.FirstName = personEntity.FirstName;
        this.LastName = personEntity.LastName;
        this.Email = personEntity.Email;
        this.Notes = personEntity.Notes;
    }

    public void MapTo(PersonEntity personEntity)
    {
        personEntity.Id = this.Id;
        personEntity.FirstName = this.FirstName ?? throw new NullReferenceException("Person.FirstName");
        personEntity.LastName = this.LastName ?? throw new NullReferenceException("Person.LastName");
        personEntity.Email = this.Email;
        personEntity.Notes = this.Notes;
    }

    public partial void MapModifiedTo(PersonEntity personEntity);

    [Create]
    public void Create([Service] IPersonPhoneList personPhoneModelList)
    {
        PersonPhoneList = _lazyLoadFactory.Create<IPersonPhoneList>(personPhoneModelList);
    }

    [Remote]
    [Fetch]
    internal async Task<bool> Fetch([Service] IPersonDbContext personContext,
                                    CancellationToken cancellationToken = default)
    {
        var personEntity = await personContext.FindPerson(cancellationToken);
        if (personEntity == null)
        {
            return false;
        }
        this.MapFrom(personEntity);
        // PersonPhoneList is lazy — loader uses this.Id (set by MapFrom)
        return true;
    }

    [Remote]
    [Insert]
    internal async Task<PersonEntity?> Insert([Service] IPersonDbContext personContext,
                                    CancellationToken cancellationToken = default)
    {
        await RunRules(token: cancellationToken);

        if(!this.IsSavable)
        {
            return null;
        }

        this.Id = Guid.NewGuid();

        var personEntity = new PersonEntity();
        this.MapTo(personEntity);
        personContext.AddPerson(personEntity);

        var phoneList = await this.PersonPhoneList.LoadAsync();
        if (phoneList != null)
        {
            _personPhoneListFactory.Save(phoneList, personEntity.Phones, cancellationToken);
        }

        await personContext.SaveChangesAsync(cancellationToken);
        return personEntity;
    }

    [Remote]
    [Update]
    internal async Task<PersonEntity?> Update([Service] IPersonDbContext personContext,
                                    CancellationToken cancellationToken = default)
    {
        await RunRules(token: cancellationToken);

        if (!this.IsSavable)
        {
            return null;
        }

        var personEntity = await personContext.FindPerson(this.Id, cancellationToken);
        if (personEntity == null)
        {
            throw new KeyNotFoundException("Person not found");
        }

        this.MapModifiedTo(personEntity);

        var phoneList = await this.PersonPhoneList.LoadAsync();
        if (phoneList != null)
        {
            _personPhoneListFactory.Save(phoneList, personEntity.Phones, cancellationToken);
        }

        await personContext.SaveChangesAsync(cancellationToken);
        return personEntity;
    }

    [Remote]
    [Delete]
    internal async Task Delete([Service] IPersonDbContext personContext,
                             CancellationToken cancellationToken)
    {
        await personContext.DeleteAllPersons(cancellationToken);
        await personContext.SaveChangesAsync(cancellationToken);
    }
}
